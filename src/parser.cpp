...
NoriObject *loadFromXML(const std::string &filename) {
    ...
    auto check_attributes = [&](const pugi::xml_node &node,
        ...
        for (auto attr : node.attributes()) {
            ...
            if (it == attrs.end())
                throw NoriException("Error while parsing \"%s\": unexpected "
        ...
        if (!attrs.empty())
            throw NoriException("Error while parsing \"%s\": missing attribute "
                                "\"%s\" in \"%s\" at %s",
    ...
    std::function<NoriObject *(pugi::xml_node &, PropertyList &, int)>
        parseTag = [&](pugi::xml_node &node, PropertyList &list,
        ...
        bool parentIsObject =
            hasParent && parentTag < NoriObject::EClassTypeCount;
        ...
        if (!hasParent && !currentIsObject)
            throw NoriException("Error while parsing \"%s\": root element "
                                "\"%s\" must be a Nori object (at %s)",
                                filename, node.name(),
                                offset(node.offset_debug()));
        ...
        if (hasParent && !parentIsObject &&
            ...
            throw NoriException("Error while parsing \"%s\": node \"%s\" "
                                "requires a Nori object as parent (at %s)",
                                filename, node.name(),
                                offset(node.offset_debug()));
        ...
        try {
            if (currentIsObject) {
                ...
                case EColor: {
                    ...
                    list.setColor(
                        node.attribute("name").value(),
                        Color3f(toVector3f(node.attribute("value").value())
                                    .array()));
                } break;
                case ETransform: {
                ...
                case ETranslate: {
                    ...
                    transform =
                        Eigen::Translation<float, 3>(v.x(), v.y(), v.z()) *
                        transform;
                } break;
                case EMatrix: {
                    check_attributes(node, {"value"});
                ...
                case ERotate: {
                    ...
                    transform =
                        Eigen::AngleAxis<float>(angle, axis) * transform;
                } break;
                case ELookAt: {
                    ...
                    Eigen::Vector3f up =
                        toVector3f(node.attribute("up").value());
                ...
                default:
                    throw NoriException("Unhandled element \"%s\"",
                                        node.name());
                };
            }
        } catch (const NoriException &e) {
            throw NoriException("Error while parsing \"%s\": %s (at %s)",
                                filename, e.what(),
                                offset(node.offset_debug()));
        }
