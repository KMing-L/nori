...
Point2f Warp::squareToTent(const Point2f &sample) {
    auto x = sample.x() < 0.5f ? sqrt(2 * sample.x()) - 1
                               : 1 - sqrt(2 - 2 * sample.x());
    auto y = sample.y() < 0.5f ? sqrt(2 * sample.y()) - 1
                               : 1 - sqrt(2 - 2 * sample.y());
    return Point2f(x, y);
}

float Warp::squareToTentPdf(const Point2f &p) {
    bool x = -1.0f <= p.x() && p.x() <= 1.0f;
    bool y = -1.0f <= p.y() && p.y() <= 1.0f;
    return x && y ? abs(1 - abs(p.x())) * (1 - abs(p.y())) : 0;
}
