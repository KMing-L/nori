...
struct WarpTest {
    ...
    std::pair<bool, std::string> run() {
        ...
        for (int i = 0; i < sampleCount; ++i) {
            ...
            int xbin =
                std::min(xres - 1, std::max(0, (int)std::floor(x * xres)));
            int ybin =
                std::min(yres - 1, std::max(0, (int)std::floor(y * yres)));
    ...
    void generatePoints(int &pointCount, PointType pointType,
        ...
        for (int i = 0; i < pointCount; ++i) {
            ...
            case Grid:
                sample =
                    Point2f((x + 0.5f) * invSqrtVal, (y + 0.5f) * invSqrtVal);
                break;
    ...
    create_microfacet_bsdf(float alpha, float kd, float bsdfAngle) {
        ...
        auto *brdf =
            (BSDF *)NoriObjectFactory::createInstance("microfacet", list);
...
struct Arcball {
    ...
    Arcball(float speedFactor = 2.0f)
        : m_active(false), m_lastPos(nori::Vector2i::Zero()),
          m_size(nori::Vector2i::Zero()), m_quat(Quaternionf::Identity()),
          m_incr(Quaternionf::Identity()), m_speedFactor(speedFactor) {}
...
class WarpTestScreen : public Screen {
  public:
    ...
    void refresh() {
        ...
        float parameterValue =
            mapParameter(warpType, m_parameterSlider->value());
        float parameter2Value =
            mapParameter(warpType, m_parameter2Slider->value());
        ...
        if (warpType != Square) {
            for (int i = 0; i < m_pointCount; ++i) {
                if (values(0, i) == 0.0f) {
                    positions.col(i) = nori::Vector3f::Constant(
        ...
        if (m_gridCheckBox->checked()) {
            ...
            for (int i = 0; i <= gridRes; ++i) {
                for (int j = 0; j <= fineGridRes; ++j) {
                    auto pt = tester.warpPoint(
                        Point2f(j * fineScale, i * coarseScale));
                    positions.col(idx++) =
                        value_scale == 0.f
                            ? pt.first
                            : (pt.first * pt.second * value_scale);
                    pt = tester.warpPoint(
                        Point2f((j + 1) * fineScale, i * coarseScale));
                    positions.col(idx++) =
                        value_scale == 0.f
                            ? pt.first
                            : (pt.first * pt.second * value_scale);
                    pt = tester.warpPoint(
                        Point2f(i * coarseScale, j * fineScale));
                    positions.col(idx++) =
                        value_scale == 0.f
                            ? pt.first
                            : (pt.first * pt.second * value_scale);
                    pt = tester.warpPoint(
                        Point2f(i * coarseScale, (j + 1) * fineScale));
                    positions.col(idx++) =
                        value_scale == 0.f
                            ? pt.first
                            : (pt.first * pt.second * value_scale);
            ...
            if (warpType != Square) {
                for (int i = 0; i < m_lineCount; ++i)
                    positions.col(i) = positions.col(i) * 0.5f +
                                       nori::Vector3f(0.5f, 0.5f, 0.0f);
            }
            m_gridShader->set_buffer("position", VariableType::Float32,
                                     {(size_t)m_lineCount, 3},
                                     positions.data());
        }
    ...
    void draw_contents() {
        ...
        proj = Matrix4f::perspective(viewAngle / 360.0f * M_PI, near_clip,
                                     far_clip,
        ...
        if (m_drawHistogram) {
            ...
            if (drawGrid) {
                ...
                m_gridShader->begin();
                m_gridShader->draw_array(nanogui::Shader::PrimitiveType::Line,
                                         0, m_lineCount);
                m_gridShader->end();
            }
            if (m_warpTypeBox->selected_index() == MicrofacetBRDF) {
                ...
                m_arrowShader->begin();
                m_arrowShader->draw_array(nanogui::Shader::PrimitiveType::Line,
                                          0, 106);
                m_arrowShader->end();
            }
        }
    ...
    void drawHistogram(const nanogui::Vector2i &pos_,
        ...
        Vector2f s =
            -(Vector2f(pos_) + Vector2f(0.25f, 0.25f)) / Vector2f(size_);
        ...
        m_histogramShader->draw_array(nanogui::Shader::PrimitiveType::Triangle,
                                      0, 6, true);
    ...
    void runTest() {
        ...
        for (int i = 0; i < tester.res; ++i) {
            maxValue =
                std::max(maxValue, (float)std::max(tester.obsFrequencies[i],
                                                   tester.expFrequencies[i]));
            minValue =
                std::min(minValue, (float)std::min(tester.obsFrequencies[i],
                                                   tester.expFrequencies[i]));
        }
        minValue /= 2;
        ...
        for (int k = 0; k < 2; ++k) {
            for (int i = 0; i < tester.res; ++i)
                buffer[i] = ((k == 0 ? tester.obsFrequencies[i]
                             ...
                             minValue) *
                            texScale;
            m_textures[k] = new Texture(
                Texture::PixelFormat::R, Texture::ComponentFormat::Float32,
    ...
    void initializeGUI() {
        ...
        m_warpTypeBox =
            new ComboBox(m_window, {"Square", "Tent", "Disk", "Sphere",
                                    "Hemisphere (unif.)", "Hemisphere (cos)",
                                    "Beckmann distr.", "Microfacet BRDF"});
